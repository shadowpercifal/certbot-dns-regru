...
2. Настройте файл с учётными данными Reg.ru:
   ...
   Если вы используете услугу для управления внешним доменным именем - укажите `service_id` услуги в файле конфигурации
   ```
   dns_regru_service_id=<SERVICE_ID>
   ```

   Вы можете получить `service_id` в ссылке на услугу для конкретного домена
