...
2. Настройте файл с учётными данными Reg.ru:
   ...
   Reg.ru теперь требует **клиентский сертификат (mTLS)** для API. Если у вас включена
   аутентификация по SSL, укажите пути к клиентскому сертификату и ключу:
   ...
   `dns_regru_cert` может указывать и на один PEM-файл, содержащий и сертификат, и закрытый
   ключ (тогда `dns_regru_key` можно не задавать).

   Для доменов, зарегистрированных в Reg.ru, плагин по умолчанию использует `servtype="domain"`.
   Если вы используете услугу «DNS-хостинг» для **внешнего** домена, задайте:
...
- Рекомендуется заключать wildcard домен (`*.example.com`) в одинарные кавычки, чтобы избежать конфликтов с консолью.
